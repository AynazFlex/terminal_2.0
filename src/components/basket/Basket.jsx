  ...
  return (
    <div className={style.basket}>
      <div className={style.basket__items}>
        {products.map(({ title, total_price }) => (
          <div className={style.basket__item} key={title}>
            <span className={style.basket__item_title}>{title}</span>
            <span className={style.basket__item_total}>{total_price} руб</span>
          </div>
        ))}
      </div>

      <div className={style.basket__total_sum}>
        К оплате:{" "}
        {products.reduce((sum, { total_price }) => total_price + sum, 0)} руб
      </div>

      <Link class={style.basket__link} to="/face-control">
        <svg
